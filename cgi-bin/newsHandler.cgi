...
@date = ();			# inizializzo l'array delle date
...
foreach $child (@children) {							# scorro tutti i nodi figli della radice
	...
	push(@date, $child->findvalue("data/giorno") . "/" . $child->findvalue("data/mese") . "/" . $child->findvalue("data/anno"));	# aggiungo all'array dei 'title' il contenuto testuale del nodo 'title'
...
for ($i=0; $i<scalar(@children); $i++) {
	...
	$row_data{date} = $date[$i];		# inserisco l'iesima data
